...
function parseListQuery(request: Request): { limit?: number; query?: string } {
  const url = new URL(request.url);
  const fields: Record<string, string> = {};
  if (url.hash) fields.fragment = "unknown";
  for (const key of new Set(url.searchParams.keys())) {
    if (key !== "q" && key !== "limit") fields[key] = "unknown";
  }
  const qValues = url.searchParams.getAll("q");
  const limitValues = url.searchParams.getAll("limit");
  if (qValues.length > 1) fields.q = "duplicate";
  if (limitValues.length > 1) fields.limit = "duplicate";

  let query: string | undefined;
  const rawQuery = qValues[0];
  if (rawQuery !== undefined && qValues.length === 1) {
    const trimmed = rawQuery.trim();
    if (trimmed.length === 0) {
      fields.q = "required";
    } else if (Array.from(segmenter.segment(trimmed)).length > QUERY_MAX_GRAPHEMES) {
      fields.q = "too_long";
    } else {
      query = trimmed;
    }
...
export async function GET(
  ...
  try {
    ...
    const result = threadTagService.listProjectTags(
      databasePath(),
      projectId,
    ...
    return threadTagErrorResponse(error, "GET /api/projects/:projectId/thread-tags");
  }
}
...
export async function POST(
  ...
  try {
    ...
    const result = threadTagService.createThreadTag(
      databasePath(),
      projectId,
    ...
    return threadTagErrorResponse(error, "POST /api/projects/:projectId/thread-tags");
  }
}
