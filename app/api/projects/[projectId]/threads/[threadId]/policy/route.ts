...
import { updateThreadPolicy } from "@/src/adapters/outbound/sqlite/public-collaboration/thread-service";
...
function invalidInput(fields: Record<string, string>): never {
  throw new CollaborationError(
    "INVALID_INPUT",
    400,
    "Policy update input is invalid.",
    { fields },
  );
}
...
async function readStrictJson(request: Request): Promise<unknown> {
  ...
  if (
    declaredLength !== null
    && (!DECIMAL_INTEGER.test(declaredLength) || Number(declaredLength) > BODY_LIMIT)
  ) {
    throw new CollaborationError(
      "BODY_TOO_LARGE",
      413,
...
export async function PATCH(
  ...
  try {
    ...
    return collaborationErrorResponse(
      error,
      "PATCH /api/projects/:projectId/threads/:threadId/policy",
    );
  }
}
