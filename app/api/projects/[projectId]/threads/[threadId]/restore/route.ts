...
function invalidInput(fields: Record<string, string>): never {
  throw new CollaborationError(
    "INVALID_INPUT",
    400,
    "Thread restore input is invalid.",
    { fields },
  );
}
...
// Restore carries no parameters: the wire DTO is the strict empty object.
...
export async function POST(
  ...
  try {
    ...
    requireEmptyObject(await readStrictJson(request));
    const result = threadLifecycleService.restoreThread(
      databasePath(),
      projectId,
      threadId,
    );
    ...
    return threadLifecycleErrorResponse(
      error,
      "POST /api/projects/:projectId/threads/:threadId/restore",
    );
  }
}
