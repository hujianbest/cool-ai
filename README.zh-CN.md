...
- **真实协作：**Agent 通过结构化 Provider 调用提出和领取任务，在项目对话讨论、显式交棒、请求 owner 决策，并从持久化状态继续。
...
Cool AI 当前不提供多用户账号或认证、公开云托管、生产部署工具、移动端优先操作、厂商原生 API、本地 Agent CLI 保证、任意 shell、敌对代码隔离、无人值守定时任务或跨重启自动推进。窄屏只提供基础查看、对话与审批，配置和并行执行仍以桌面端为主。
