...
- 用户已明确决定（2026-08-14，指示“按建议优化”）：首次正式发布前的连续开发中，**默认豁免** spec review、architecture review 与 `hf-review`（沿用 2026-08-09 豁免）；**恢复 `hf-code-review` 代码门评审**，按下列范围选择性执行（取代原“全部 review 豁免”）。
- 命中以下任一条件的切片，`implement` 完成后必须执行 `hf-code-review`（至少 Standards 轴；Spec 轴按 hf-code-review 流程判断），不得豁免：
  - schema 变更（`CURRENT_SCHEMA` identity 或 DDL 变化）；
  - 凭据、沙箱、工作区 verified-handle 等安全边界；
  - 跨 owner 写、共享 outbox/投影/审计写入点；
  - 超过 8 张任务票的切片（含基础重构）。
- 不触碰上述条件的纯 UI、机械、文档切片继续适用基础豁免，不强制 `hf-code-review`。
- 无论豁免与否，都不得伪造 review 结论、确认行或评审文件；因豁免跳过 review 时，应在对应 `progress.md` 记录“项目级 review 豁免”及日期后继续下一阶段。
- 豁免不免除 TDD、聚焦测试、最终全量测试、类型检查、生产构建、受影响浏览器冒烟、axe、真实 UI 验收、安全边界、任务票勾选、commit 与 push。
- 首次正式发布前必须由用户明确决定最终评审策略；调整前须修改本节及相关技能描述，不得静默恢复或继续豁免。
...
- 同时满足以下全部条件的切片可走轻量级路径：任务票 ≤3 张；不触碰 schema（`CURRENT_SCHEMA` identity/DDL）、安全边界或跨 owner 写；局限在单一模块/接缝内。
- 轻量级路径：`to-spec` 与 `to-architecture` 合并为一页 `architecture.md`（含规格要点与测试缝），跳过 `to-tickets`（任务票直接列于 `progress.md` 或 `tickets.md` 简表）；仍须落盘假设台账与 `progress.md`，仍须完成全部测试/构建/冒烟/验收门禁。
- 存量热修沿用“仅架构地图”先例（`product/architecture.md` 只建地图，不强制完整产品层）。
- 超出轻量级条件的切片必须走完整主链（to-spec → to-architecture → to-tickets）；轻量级不是缩小验收范围的口子。
...
- 单个实现 subagent 通常只承接 2～5 张同模块、同接缝的相关任务票；默认墙钟预算为 30 分钟，按切片类型浮动：领域/机械/查询类 30 分钟，UI、浏览器验收与集成类 45～60 分钟（验收段写入量大的切片允许 60 分钟）。超出预算必须停下报告并拆段，不得因无人值守而无限续跑。
...
- 机械性改动（identity/断言号同步、测试夹具补字段、纯配置、纯文档、无行为变化的迁移性编辑）可豁免“行为性 RED”，直接以验证/回归测试收尾；豁免须在票或 `progress.md` 记录理由（先例：007 纯文档、032 T-01 identity 同步）。不得把行为性功能改动伪装成机械改动。
...
- `hf-code-review` 已按选择性评审策略完成（命中强制条件的切片）或已记录豁免（未命中条件的切片）；不得伪造评审记录；
- 用户可感知工作已记录演示验收；
- `ship` 阶段完成（评审与验收记录齐备）；
- 产品、上下文与假设记录已更新；
- 工作树不存在意外工件；
