...
- **重构不属于该循环。**它属于评审阶段（参见 `hf-code-review` 技能），而不是红 → 绿实现循环。

## 机械性改动豁免

行为性 RED（测试因行为缺失而失败）只适用于**引入或改变行为**的改动。以下机械性改动可豁免行为性 RED，直接以验证/回归测试收尾，但须在票或 `progress.md` 记录豁免理由：

- 标识号/断言号同步（如 schema identity 引用、测试内硬编码号）；
- 测试夹具补字段/形状对齐（夹具追平既有契约，非生产行为）；
- 纯配置、纯文档、无行为变化的迁移性编辑（重命名、移动、注释）。

豁免不适用于任何会改变用户可观察行为的改动——行为性功能改动不得伪装成机械改动来绕过 RED。豁免后仍须运行聚焦测试/回归测试证明改动不破坏既有行为（先例：007 纯文档、032 T-01 identity 同步）。
