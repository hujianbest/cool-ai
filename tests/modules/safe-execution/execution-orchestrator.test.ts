...
beforeEach(async () => {
  ...
  const modules = import.meta.glob<AdvanceModule>(
    "../../../src/adapters/outbound/sqlite/safe-execution/action-orchestrator.ts",
  );
  const load = modules["../../../src/adapters/outbound/sqlite/safe-execution/action-orchestrator.ts"];
  expect(load, "T-16 action orchestrator must exist").toBeTypeOf("function");
  advance = await load();
  approvals = await import(
    "@/src/adapters/outbound/sqlite/safe-execution/execution-approval-service"
  ) as ApprovalModule;
});
