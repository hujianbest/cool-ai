...
import { discardExecutionAction } from "@/src/adapters/outbound/sqlite/safe-execution/execution-actions";
...
beforeEach(async () => {
  ...
  try {
    guard = await import("@/src/adapters/outbound/workspace/path-guard") as GuardModule;
    listTool = await import("@/src/adapters/outbound/workspace/file-tools") as ListModule;
  } catch {
...
describe("verified-handle bounded list", () => {
  it("wires production file and stage actions without the Node path fallback", async () => {
    const moduleId = "@/src/adapters/outbound/workspace/windows-verified-execution-adapter";
