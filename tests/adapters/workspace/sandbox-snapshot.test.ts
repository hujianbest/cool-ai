...
import { preflightSandbox, type SandboxPreflightResult } from "@/src/adapters/outbound/workspace/sandbox-preflight";
...
type SnapshotModule = typeof import("@/src/adapters/outbound/workspace/sandbox-snapshot") & {
...
beforeEach(async () => {
  ...
  const moduleId = "@/src/adapters/outbound/workspace/sandbox-snapshot";
  try {
    snapshot = await import(/* @vite-ignore */ moduleId) as SnapshotModule;
    actions = await import("@/src/adapters/outbound/sqlite/safe-execution/execution-actions") as ActionModule;
  } catch {
