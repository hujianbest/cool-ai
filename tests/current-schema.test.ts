...
describe("current canonical schema", () => {
  ...
  it("bootstraps a missing database directly at identity 9", () => {
  ...
  it("bootstraps an existing empty SQLite database to the same canonical inventory", () => {
    ...
    try {
      expect(empty.prepare("PRAGMA user_version").get()).toEqual({ user_version: 9 });
  ...
  it("rolls back a failed bootstrap before the current identity is published", () => {
    ...
    try {
      expect(recovered.prepare("PRAGMA user_version").get()).toEqual({ user_version: 9 });
    } finally {
      recovered.close();
    }
  });
