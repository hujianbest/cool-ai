...
type ReviewSchemaModule = typeof import("../src/server/review/review-schema");
type ReviewOrchestratorModule = typeof import("../src/server/review/review-orchestrator");

const schemaModules = import.meta.glob<ReviewSchemaModule>(
  "../src/server/review/review-schema.ts",
);
...
describe("review product-domain redaction", () => {
  it("builds provider egress from only the selected reviewer private config and allowlisted public material", async () => {
    const load = schemaModules["../src/server/review/review-schema.ts"];
  ...
  it("fails closed on redaction failure and writes no decision", async () => {
    const db = database();
    const load = orchestratorModules["../src/server/review/review-orchestrator.ts"];
    expect(load).toBeTypeOf("function");
