...
export function execV7Fixture(
  ...
  if (options.validate !== false) assertV7Fixture(database);
  return threads;
}
...
export function assertV7Fixture(database: DatabaseSync): void {
  ...
  if (validation !== null) {
    throw new Error(`V7 fixture graph is invalid: ${validation}`);
  }
}
...
export function execV7TupleStatements(
  ...
  const validation = validateV7(database);
  if (validation !== null) {
    throw new Error(`V7 fixture tuples are invalid: ${validation}`);
  }
}
