...
`RecoveryFileDto={position,path,pathKey,oldExists,oldHash,newHash,status,isMismatch}`；`path/pathKey`只允许canonical workspace-relative值，hash与status是恢复判定元数据，不含文件正文或内部root/owned-file descriptor。
...
- manual recovery: detail loading显示只读恢复skeleton且全部mutation disabled；detail error只给刷新，不降级为普通conflicted controls；完整journal/phase/三hash到达后显示摘要。`mismatchPathKey`非空时仅对应file标为不匹配；为空时固定显示“整体不匹配”，所有file mismatch=false，不伪造路径。files empty表示0个可列journal row而不是已恢复，分页error保留已读项但resolve保持disabled直到detail tuple有效。manifest变化返回`RECOVERY_MANIFEST_MISMATCH`并刷新observed摘要，不保留“已确认”状态；成功后polite live region，`recovered_old`聚焦retry，`recovered_new`聚焦merged heading，`abandon`聚焦stopped heading。
...
- [ ] T-47 收口只经公开行为的真实 browser smoke harness (覆盖: FR-1, FR-2, FR-3, FR-8, FR-9, FR-11, FR-14, NFR-1, NFR-4, NFR-5) — 判据: `npm test -- tests/execution-browser-smoke.test.ts`先红后绿；smoke 不拦截 start/advance/stage/merge、detail、recovery files及既有 `/recovery/resolve` 路由，不直接写业务 SQLite 或复制 canonical 文件来合成状态，双 execution 从 start 经真实 provider、工具/验证、stage 到至少一次 merge 只走产品公开 API；stale/conflict由公开行为触发，manual recovery固定由merge fault injection→GET detail/files→既有resolve，刷新/重启仍可见恢复tuple；provider 健康探针不计入 execution 并发断言
- [ ] T-48 运行S-5 desktop+narrow smoke/demo，仅验证前序已实现行为 (覆盖: FR-1..FR-14, NFR-1..NFR-5) — 判据: 本任务不得新增或修改product code/test行为；README、`npm test`、`npm run build`、`npm run smoke:execution`通过，并留真实provider双Agent不相交合入、重复/边界/standing+one-shot、stale/conflict/manual recovery及desktop/narrow证据

任务覆盖索引：FR-1→T-2/3/17/20/27/28/31/40/47–48；FR-2→T-1/2/5/16/22–26/28/30–31/40/43–48；FR-3→T-8–10/12/14–19/28/31–32/37/41/44/47–48；FR-4→T-6–10/16/31/33–34/36–37/40/48；FR-5→T-6/7/10/16/31/34–37/40–41/48；FR-6→T-11–13/16/19/25/27/29/31/43–44/48；FR-7→T-19/20/26/29/31/37/41–42/45/48；FR-8→T-7/11/15/18–23/29/31/33–42/44–48；FR-9→T-19–23/25/29/30–31/33/35/38–39/45–48；FR-10→T-1/5/6/8–14/17/19/24/26/28/31–32/36/40–41/43/46/48；FR-11→T-1/2/4/5/7/8/10/13/14/16/18/21–26/31/36–48；FR-12→T-4/5/18/20/23–25/28/30–31/43/46/48；FR-13→T-1/4/5/9/11–15/18/22–26/29/31/43–46/48；FR-14→T-2/11/27–30/45–48。NFR-1→T-1–3/5/7/10/16–24/28/30–48；NFR-2→T-6–17/19/21/26/29/31/33–41/43/46/48；NFR-3→T-6/8–15/25/27/29/30–35/37/43–44/46/48；NFR-4→T-1/2/4/5/7–9/11–18/20–26/31/36–48；NFR-5→T-2/27–30/45–48。
