...
默认adapter wiring是server build graph的一部分，不是运行时可选插件。`sandbox-preflight.ts` 以静态相对import直接连接 `./windows-native-read-adapter` 的 `createWindowsNativeReadAdapter`；禁止 `import("@/...")`、拼接specifier、`require` fallback或catch module resolution。Node/Vitest与Next因此都在加载/构建阶段解析同一模块，production `createDefaultSandboxFsAdapter()` 只做runtime选择与factory调用：

1. 纯runtime selector先检查 `process.platform==="win32"`、`process.arch==="x64"` 与既有支持矩阵；unsupported OS/arch直接抛typed `SANDBOX_UNVERIFIABLE`，factory调用次数=0。
2. supported runtime直接调用静态wired factory并返回同一verified-handle adapter。factory抛出的 `WindowsNativeError` 或其他带 `code="SANDBOX_UNVERIFIABLE"` 的typed native/ABI/symbol/capability错误原样传播，保留其typed reason/cause供server诊断，不改写成“module unavailable”。
3. factory若抛无typed未知错误，只在factory边界包装为typed `SANDBOX_UNVERIFIABLE`，message固定为adapter construction failed并保留`cause`；不把构造/ABI错误描述成模块解析失败。静态模块自身若不存在或无法被resolver加载，应让Vitest/Node import或Next build直接失败，不能被业务catch伪装成运行时422。
4. `createDefaultSandboxFsAdapter()` 保持现有async返回契约，供 `preflightSandbox`、snapshot/cleanup与production sandbox executor共用同一factory；测试依赖注入仍只覆盖显式fault boundary，默认happy path必须走该wiring。任何失败都无path-level adapter fallback、无普通`fs`遍历、无部分snapshot。
5. `koffi` 继续只由 `next.config.ts serverExternalPackages:["koffi"]` 外置；Next server build不得把native binary打入client或edge bundle。构建回归同时验证server module可解析、external仍存在且默认factory在Node server runtime可构造。
...
| unsupported platform或default factory/native ABI/identity/final-path不可验证 | typed reason保持`SANDBOX_UNVERIFIABLE`且不误报module unavailable；production attempt/action/execution=`failed(error=SANDBOX_UNVERIFIABLE)`，receipt=completed 422；直接preflight同code拒绝；不得暴露ready/201，canonical hash不变，无path fallback |
| static adapter module/`koffi`无法被Node/Vitest/Next解析 | import/test/build直接失败，不进入业务catch、不生成伪422或业务receipt | 修复build graph/`serverExternalPackages`后重跑；不得运行降级adapter |
...
4. 所有path达到post后，在DB commit事务之前再次逐路径核对post和整体post manifest；事务取得project lock后再做一次完整post核对，随后同一事务插入task result、execution=merged并把journal置`db_committed`，但merge action保持running、receipt保持pending。所有execution/result/list read在发现incomplete journal时必须先运行既有read barrier/helper，未恢复前不可观察私有merged/result；已是durable `manual_recovery` 时则按下文manual-aware只读协议返回恢复摘要，不再次调用helper。commit返回后立刻再次逐路径核对post和整体manifest；相符时第二事务把journal=`completed`、action succeeded、最后完成merge receipt并解除barrier。这些检查与DB commit间仍存在不可消除的外部race；post-commit检查检测到时立即执行步骤6。若进程恰在commit后/post-check前崩溃，既有 `recoverIncompleteMergeJournals` read barrier/helper只依据原journal、merge action与receipt做恢复，不创建新的public recover route、operation或action。
...
6. 外部不匹配或journal存在后任一native/capability/identity/cleanup结果不可验证时，`enterManualRecovery` 在同一事务原子设置journal=`manual_recovery`、execution=`conflicted,manual_recovery_required=1`，保存current overall manifest、mismatch phase及nullable `mismatch_path_key`，令原merge action=`failed(code=MANUAL_RECOVERY_REQUIRED)`并完成原merge receipt为409。path-specific detector必须把已经canonical relative-path validator确认的path key随本次enter事务写入；overall manifest、capability、cleanup或无法归因单一路径时固定写NULL。若尚未DB commit则task result从未写；若在步骤4的私有commit后才检测，则同一补偿事务删除仅与该journal FK关联且从未越过read barrier的task result、清除merged_at。对外可观察的merged/任务结果提交次数仍为0。平台不再自动写canonical。普通advance/retry/control/merge/stop全部409 `MANUAL_RECOVERY_REQUIRED`，唯一公开写入口是既有 `/recovery/resolve` endpoint。
...
manual recovery 是阻断 mutation 的终态，不是阻断 owner 读取恢复事实的理由。`GET /api/executions/:executionId` 与 `GET .../recovery/files` 使用同一 manual-aware read barrier：

1. barrier先按project唯一未解决journal分类并保留其 `(project_id,execution_id,journal_id,status)`。manual 200例外仅在该journal status=`manual_recovery`、route `executionId` 与journal execution相等、且同一read transaction读到execution `(project_id,execution_id)`相等并有 `manual_recovery_required=1` 时成立；三者任一不符禁止拼tuple。同project sibling execution detail以及project execution list继续409 `MERGE_RECOVERY_REQUIRED`；请求execution标manual但不存在精确未解决journal，或journal/manual flag互相矛盾，均500 `MERGE_INVARIANT_FAILED` fail-closed。
2. 精确manual journal在入口已durable时跳过 `recoverIncompleteMergeJournals`，在单个SQLite read transaction读取execution、同一journal与counts并返回200。该分支canonical写=0、DB写=0，不改变journal/execution/action/receipt/version/observed hash。无project unresolved journal且execution flag=0时才走普通detail。
3. status=`prepared|applying|db_committed|rolling_back|rolling_forward`时才允许调用既有安全reconcile helper。GET触发的唯一canonical写是D-5已经定义的conditional rollback `post→old` 或roll-forward `old→post`；每个path写前后仍验证identity/hash，禁止新增、放宽或无条件恢复。helper遇任何native/capability/identity/cleanup不可验证均沿既有D-5在同一事务enter durable manual、终结原action并完成原receipt 409，然后抛 typed `MANUAL_RECOVERY_REQUIRED`；manual-aware barrier只捕获这个已经持久的结果并重新执行第1步。若精确匹配route execution，同一次GET返回200 manual detail，不把helper内部409/`SANDBOX_UNVERIFIABLE`暴露为GET 422/409；若journal属于sibling则仍返回project barrier 409。
4. helper安全完成后重新分类：completed/resolved返回普通detail；若连manual/完成终态都无法持久化或DB/schema读取失败，返回500 `INTERNAL_ERROR|MERGE_INVARIANT_FAILED`，保留barrier且不返回部分detail，后续GET只可按durable journal重试。响应超过detail/page上限分别返回413 `RESPONSE_LIMIT_EXCEEDED`；已经安全完成的reconcile不因序列化失败回滚或重放。
5. 并发GET、restart helper、lease reconcile、owner resolve与late merge finalizer只以journal/action CAS争胜：helper只可更新incomplete status，`manual_recovery|resolved_*|abandoned|completed`均不可被late helper覆盖。协调后detail在新read transaction重新按三方identity取得一致快照，因此只会看到同一journal的完整manual tuple或resolution后的普通tuple；resolve先赢并清除/替换unresolved journal时不得沿用旧journal生成200 manual，旧recovery-file cursor失效。若新的unresolved journal属于同execution，必须重新按新journal完整tuple读取；属于sibling或已无journal但flag不一致则分别409/500。
6. manual detail固定返回 `execution.manualRecoveryRequired=true` 及 `recovery={required:true,journalId,journalStatus:"manual_recovery",mismatchPhase,mismatchPathKey,oldManifestHash,postManifestHash,observedManifestHash,allowedResolutions}`；全部hash必须是已持久64字符小写hex。`mismatchPathKey` 明确nullable且不参与resolution可见性的完整性判定：非NULL时必须是该journal内已验证规范path key；NULL表示overall/capability/cleanup不匹配，UI固定显示“整体不匹配”。可增长的per-file差异不内嵌detail，只由 `GET .../recovery/files` 分页。
7. recovery file page固定 `limit=1..20`、稳定 `(position,pathKey)`、最多100 rows、response≤512KiB；签名cursor绑定 `executionId+journalId+route`，不得跨journal/restart后的resolution重放。每项只暴露规范workspace-relative `path/pathKey`、position、old exists/hash、post hash、journal file status与`isMismatch`。post hash必须从始终持久的staged file hash读取，并与durable-new descriptor hash一致后才返回；不得依赖可能为NULL的 `post_target_ref_json.sha256`。`mismatchPathKey=NULL` 时所有file `isMismatch=false`。不得返回绝对workspace/sandbox/journal root、backup/temp locator、file identity、原始bytes、diff正文、credential或模型私有输入；path须通过既有canonical relative-path validator与UTF-8/长度边界，任一stored identity/hash/path不一致整页500 fail-closed且不回显原始值。
8. `execution_merge_files.status` 的唯一合法全集在v5 DDL、server domain tuple、`RecoveryFileDto` schema/parser与UI exhaustive switch中统一为 `pending|temp_ready|applied|rolled_back|rolled_forward|verified`，不得用 `string`、fallback label或passthrough放宽。唯一runtime tuple/schema/type放在 `src/shared/execution-contracts.ts`，merge/read service与UI共同导入；v5 migration strict validator测试逐值断言DDL enum与该tuple相等，禁止再维护局部缩水enum。v5 DDL保持事实源：`pending`要求canonical temp/post target均NULL，其他五种要求二者均非NULL；`temp_ready`表示durable canonical temp及post descriptor已持久、尚未记录replace完成，是manual recovery完全合法的中间状态。公开DTO原样返回上述稳定枚举，但只作journal历史诊断，不单凭file status宣称canonical当前为old/post；未知、大小写变体、NULL或与DDL row-shape不一致统一500 `INTERNAL_ERROR`且不回显原值。

| public status | UI只读文案 | 安全含义 |
|---|---|---|
| `pending` | 待准备 | durable-new已登记，canonical temp/post descriptor尚未登记 |
| `temp_ready` | 临时文件已准备 | temp/post descriptor已登记，是否replace必须看journal/manual manifests |
| `applied` | 已记录应用 | replace已记录，不代表manual现场仍等于post |
| `rolled_back` | 已记录回退 | helper记录回退完成，不替代当前manifest校验 |
| `rolled_forward` | 已记录前滚 | helper记录前滚完成，不替代当前manifest校验 |
| `verified` | 已验证 | journal步骤已验证，仍以overall/manual tuple决定resolution |

上述放行只适用于detail与recovery-file只读入口。advance/control/retry/stop/approval/stage/merge等普通mutation在 `manualRecoveryRequired` 下仍409 `MANUAL_RECOVERY_REQUIRED`；唯一写入口仍是带expectedVersion与observed hash CAS的既有 `/recovery/resolve`。
...
- `GET /api/executions/:executionId` → `ExecutionDetailResponse`（只有标量/摘要，不内嵌可增长集合；仅route/journal/execution identity与manual flag精确匹配的durable manual recovery返回200 recovery tuple）
...
- `GET /api/executions/:executionId/recovery/files?after=<position,pathKey>&limit=1..20` → `CursorPage<RecoveryFileDto>`（cursor绑定execution+journal，仅精确manual identity状态只读）
...
type ExecutionDetailResponse = {
  ...
  recovery:{required:boolean;journalId:string|null;journalStatus:string|null;
    oldManifestHash:string|null;postManifestHash:string|null;observedManifestHash:string|null;
    mismatchPhase:string|null;mismatchPathKey:string|null;
    allowedResolutions:Array<"recovered_old"|"recovered_new"|"abandon">};
};
```

`RecoveryMergeFileStatus="pending"|"temp_ready"|"applied"|"rolled_back"|"rolled_forward"|"verified"`；`RecoveryFileDto={position,path,pathKey,oldExists,oldHash,postHash,status:RecoveryMergeFileStatus,isMismatch}`。`path/pathKey`只允许canonical workspace-relative值，hash与status是恢复判定元数据，不含文件正文或内部root/owned-file descriptor；parser与UI必须对六值exhaustive，unknown fail closed。
...
| GET见durable manual journal且route/journal/execution identity+flag精确匹配 | 不运行helper、不改manual tuple；200 detail+分页recovery files | 0 | owner用返回的observed hash调用既有resolve；普通mutation仍409 |
| GET请求同project sibling、无journal或identity/flag不一致 | sibling detail/project list=409 `MERGE_RECOVERY_REQUIRED`；不一致=500 `MERGE_INVARIANT_FAILED`；不返回recovery tuple | 0 | resolve/新journal后重新分类，禁止混用旧journal |
| GET见incomplete journal且安全reconcile成功 | helper按D-5 conditional rollback全旧或roll-forward全新并终态化原journal/action/receipt | 只可能D-5允许的conditional canonical写 | 重读一致快照后200普通detail；不创建recover operation/action |
| GET reconcile检测external/native/capability/identity/cleanup不可验证 | 原子enter manual、原action failed、原receipt completed 409；精确route重读后200 recovery detail；若终态无法持久化则500且barrier保留 | 检测后零自动覆盖；不返回部分detail | sibling仍409；late helper/finalizer CAS不可覆盖manual/resolution |
...
- 默认adapter wiring: `tests/execution-security-integration.test.ts` 不注入platform，直接以默认 `preflightSandbox→buildSandboxSnapshot` 复现当前两个adapter unavailable RED并在GREEN完成真实快照。扩展 `tests/sandbox-preflight.test.ts tests/windows-native-loader.test.ts`：Node/Vitest import同一静态factory成功；supported Windows x64只构造一次；typed native/ABI/symbol错误保留原 `SANDBOX_UNVERIFIABLE` reason/cause；unknown factory error仅包装construction failed；unsupported platform/arch不调用factory且同code fail closed；全分支普通path `fs` fallback调用数=0。`npm run build`验证Next server graph可解析且`koffi`仍在`serverExternalPackages`、不进入client/edge bundle；production build后native load smoke继续通过。
...
- manual recovery只读闭环: 扩展 `tests/execution-merge-route.test.ts` 从公开merge fault得到durable manual状态，随后只能经公开 `GET /api/executions/:executionId` 取得完整recovery tuple、分页files并提交既有resolve，不得直接读业务SQLite拼 observed hash。扩展 `tests/execution-read-api.test.ts`：RED固定复现manual detail被read barrier 409；GREEN覆盖manual-at-entry canonical/DB写=0、incomplete-at-entry仅执行D-5 conditional reconcile，以及helper在native/capability/identity/cleanup不可验证时先完成原receipt 409并enter manual、同execution GET捕获后重读200。加入同project sibling detail/project list 409、journal/execution/manual flag错配500、resolve并发换journal/无journal、restart/refresh；100 files按20-item journal-bound cursor分页，path-specific mismatch精确一项`isMismatch=true`，overall/capability/cleanup的nullable mismatch path令全部false且UI显示整体不匹配；post hash只取一致的staged/durable-new hash。保留非法/绝对/secret path fail-closed及256/512KiB envelope。扩展 `tests/execution-recovery-ui.test.tsx` 覆盖summary/files loading/empty/error、分页、整体不匹配文案、resolve disabled直到完整tuple、刷新后恢复panel以及409 manifest变化。T-46命令同时回归 `tests/merge-recovery.test.ts tests/merge-fault-injection.test.ts`，用并发barrier证明manual/resolved胜者不被late helper/finalizer覆盖且普通mutation继续409。
- recovery file状态契约: 扩展 `tests/execution-merge-route.test.ts tests/execution-read-api.test.ts`，RED必须由公开merge fault自然停在DDL合法`temp_ready`（不得测试后手改成其他状态），detail后GET recovery files稳定复现500；GREEN返回200且post hash/mismatch/path边界不变。表驱动覆盖DDL六个合法值在shared domain tuple、DTO schema/parser、分页与数据库关闭重开后逐字往返；`tests/migrations-v5.test.ts` 从strict DDL validator提取/核对同一六值，防止schema再次漏值。单独schema/domain边界注入unknown、大小写变体、NULL及row-shape矛盾，全部500 fail-closed且响应/日志不回显原值。扩展 `tests/execution-recovery-ui.test.tsx` 对六值逐一断言D-5文案，真实`temp_ready`页面可渲染并保持resolve guard；unknown只进入files error。`tests/merge-fault-injection.test.ts` 保持真实fault产生`temp_ready`与manual journal的低层回归。
...
### 10.1 `[verification-only]` 任务证据契约

`[verification-only]` 只用于“不新增或修改product code/test行为、只运行既有验证”的已计划任务，防止 gate 诱导为不存在的行为缺口伪造RED。唯一机器语法是任务ID后恰一个ASCII空格、全小写精确marker；marker后只能EOL，或恰一个ASCII空格再接首字符非空白的非空描述。对ID后rest的等价识别正则是 `^ \[verification-only\](?:$| [^ \t\r\n].*)$`，本任务实例为 `- [x] T-50 [verification-only] 运行...`。

- `read_tasks` 返回任务ID、done与verification-only flag；仅上述完整语法置flag。ID后双空格/tab、`[Verification-only]`、`[VERIFICATION-ONLY]`、`[verification_only]`、拼写错误、`[verification-only]suffix`、marker后tab/双空格/只有空格、marker放在描述后或其他近似写法全部只是普通文本，该任务仍按普通RED/GREEN校验，不做宽松大小写/substring匹配。
- `check_tasks_done` 对两类任务完全一致：未勾选一律阻断。tier-1没有计划任务行，不支持该marker，仍要求RED/GREEN。
- 普通任务也同步改为按文件名解析出的完整label精确相等，既有exit语义不变：RED只接受label=`t<N>-red`且至少一份log exit非0，GREEN只接受label=`t<N>-green`且最新一份log exit 0；`t<N>-redo`、`t<N>-red-extra`、`t<N>-greenish`、`t<N>-green-extra`均不得计入。错误marker回落后同样使用这条严格普通规则。
- verification-only候选label必须匹配完整边界 `^t<N>-[a-z0-9][a-z0-9-]*$`；因此`t50-readme`可用，而`t500-proof`、`t50`、`t50-`、`t50--proof`及其他task/suite/smoke/demo不可用。候选日志还必须通过strict run envelope，并且至少一份exit=0；多份中任一严格envelope exit0即可。
- strict run envelope逐文件按原始UTF-8文本校验：首行必须恰为`# hf-gate-run`；全文必须恰一条`# label: X`且非空X与文件名解析出的完整label完全相等；恰一条`# command: C`且C去除首尾空白后非空；恰一条`# started: S`且S可按ISO 8601解析并含UTC `Z`或明确offset；恰一条`# exit: N`且N是十进制整数，该行必须是最后一个非空行。缺失、重复、label mismatch、非法/naive started、无command、无exit、非整数exit、exit后还有非空内容全部无效；仅文件名与`# exit: 0`不构成机器证据。
- verification-only正例单测必须调用 `run_gate(["run", ...])` 真实生成信封后再执行check，不得只用fixture helper拼正例；负例才逐项构造缺失/重复/mismatch/非法started/非终端exit。该结构校验不是密码学签名，无法识别蓄意完整仿造；人工创建或编辑出结构完整的evidence仍由“编辑evidence=造假”的流程红线禁止，文档不得宣称机械证明作者身份。
- 通过输出明确写“verification-only 机器证据”，不伪称red→green。strict envelope是verification-only新增要求；普通RED/GREEN本任务只收紧完整label相等并保留既有exit语义，不能因修例外放宽或误收近似label。
- 该例外仅影响 `check --to verify|ship` 中逐任务证据分支，不豁免任务完成、plan/design/code review、suite新鲜度、smoke、demo或demo acceptance。T-50保持已勾选且不新增第二个最终验证任务；T-51完成后进入verify，仍由hf-verify按既有规则重跑suite/smoke并执行独立代码评审。
- 实现与文档必须同步 `.cursor` / `.opencode` 两套 `hf-workflow/scripts/{hf_gate.py,test_hf_gate.py}`、`hf-workflow/SKILL.md` 与 `hf-build/SKILL.md`，镜像语义和测试逐字一致，避免一侧放宽。
...
- manual recovery: detail loading显示只读恢复skeleton且全部mutation disabled；detail error只给刷新，不降级为普通conflicted controls；完整journal/phase/三hash到达后显示摘要。`mismatchPathKey`非空时仅对应file标为不匹配；为空时固定显示“整体不匹配”，所有file mismatch=false，不伪造路径。files按D-5六值exhaustive映射显示只读文案，其中`temp_ready`明确显示“临时文件已准备”而非错误或“已应用”；unknown状态整页error且resolve disabled。files empty表示0个可列journal row而不是已恢复，分页error保留已读项但resolve保持disabled直到detail tuple有效。manifest变化返回`RECOVERY_MANIFEST_MISMATCH`并刷新observed摘要，不保留“已确认”状态；成功后polite live region，`recovered_old`聚焦retry，`recovered_new`聚焦merged heading，`abandon`聚焦stopped heading。
...
- [x] T-46 放行durable manual recovery的安全详情读取与公开resolve闭环 (覆盖: FR-2, FR-8, FR-9, FR-10, FR-11, FR-12, FR-13, FR-14, NFR-1, NFR-2, NFR-3, NFR-4, NFR-5) — 判据: `npm test -- tests/execution-merge-route.test.ts tests/execution-read-api.test.ts tests/execution-recovery-ui.test.tsx tests/merge-recovery.test.ts tests/merge-fault-injection.test.ts`先红后绿；RED必须从公开merge fault得到manualRecoveryRequired后，公开GET detail稳定409 `MERGE_RECOVERY_REQUIRED`且owner无observed hash，禁止直接查业务SQLite补值；GREEN必须由manual-aware barrier在durable manual且route/journal/execution project+execution identity及flag精确匹配时200返回bounded recovery tuple，分页files后可调用既有resolve，普通mutation仍409；manual-at-entry GET canonical/DB写=0，只有incomplete-at-entry可执行既有conditional rollback/roll-forward；helper遇native/capability/identity/cleanup不可验证统一enter durable manual、原receipt 409，同execution GET捕获后重读200而非422/409；覆盖sibling detail/project list仍409、identity/flag错配500、resolve并发换journal/无journal、path-specific与nullable overall mismatch、staged/durable-new post hash、100-row journal cursor、relative path/secret边界、refresh/重启及late CAS，确保不混tuple且UI整体不匹配/loading/empty/error/disabled/focus符合§11
- [x] T-47 统一merge recovery file合法状态的公开DTO与UI映射 (覆盖: FR-2, FR-9, FR-10, FR-11, FR-13, FR-14, NFR-1, NFR-2, NFR-3, NFR-4, NFR-5) — 判据: `npm test -- tests/execution-merge-route.test.ts tests/execution-read-api.test.ts tests/execution-recovery-ui.test.tsx tests/merge-fault-injection.test.ts tests/migrations-v5.test.ts`先红后绿；RED必须由公开merge fault生成真实DDL合法`temp_ready` manual file，GET recovery files稳定500且不得手工改业务SQLite合成该happy path；GREEN必须以shared tuple统一v5 DDL/domain/DTO schema+parser/UI exhaustive switch的六值全集，真实`temp_ready`分页200并渲染“临时文件已准备”，resolve guard与post hash/mismatch/path边界不变；表驱动逐个DDL合法状态、20-item分页和数据库重开，unknown/大小写/NULL/row-shape矛盾全部500且不回显，不用string/fallback/passthrough放宽
- [x] T-48 收口只经公开行为的真实 browser smoke harness (覆盖: FR-1, FR-2, FR-3, FR-8, FR-9, FR-11, FR-14, NFR-1, NFR-4, NFR-5) — 判据: `npm test -- tests/execution-browser-smoke.test.ts`先红后绿；smoke 不拦截 start/advance/stage/merge、detail、recovery files及既有 `/recovery/resolve` 路由，不直接写业务 SQLite 或复制 canonical 文件来合成状态，双 execution 从 start 经真实 provider、工具/验证、stage 到至少一次 merge 只走产品公开 API；stale/conflict由公开行为触发，manual recovery固定由merge fault injection→GET detail/files（含真实`temp_ready`）→既有resolve，刷新/重启仍可见恢复tuple；provider 健康探针不计入 execution 并发断言
- [x] T-49 修复默认Windows verified adapter的静态factory wiring与typed错误保真 (覆盖: FR-4, FR-5, FR-8, FR-10, FR-11, NFR-1, NFR-2, NFR-3, NFR-4) — 判据: `npm test -- tests/execution-security-integration.test.ts tests/sandbox-preflight.test.ts tests/windows-native-loader.test.ts`先红后绿且`npm run build`通过；RED固定为security integration不注入adapter时两处default preflight报unavailable；GREEN必须以Node/Vitest/Next一致解析的静态相对factory wiring完成真实preflight/snapshot，supported factory可用，native构造/ABI/symbol typed `SANDBOX_UNVERIFIABLE` reason/cause不被改写为module unavailable，unknown construction与unsupported platform/arch分别按D-2唯一映射，所有失败path fallback=0；build验证`koffi`仍由`serverExternalPackages`外置且server-only native模块不进client/edge
- [x] T-50 [verification-only] 运行S-5 desktop+narrow smoke/demo，仅验证前序已实现行为 (覆盖: FR-1..FR-14, NFR-1..NFR-5) — 判据: 本任务不得新增或修改product code/test行为；README、`npm test`、`npm run build`、`npm run smoke:execution`通过，并留真实provider双Agent不相交合入、重复/边界/standing+one-shot、stale/conflict/manual recovery及desktop/narrow证据
- [x] T-51 修复verification-only任务的机械证据门禁并同步镜像文档 (覆盖: NFR-4) — 判据: `python -m pytest .cursor/skills/hf-workflow/scripts/test_hf_gate.py .opencode/skills/hf-workflow/scripts/test_hf_gate.py`先红后绿；RED新增真实T-50场景：已勾选精确marker、无red/green，正例必须调用`run_gate run`生成`t50-*` exit0严格信封，当前`check --to verify`稳定失败；GREEN marker只匹配ID后恰一ASCII空格+精确小写token，右侧仅EOL或一ASCII空格+非空描述，双空格/tab/suffix/大小写/拼写/下划线/后置marker均回落普通RED/GREEN；verification日志要求任务边界label与文件名/header一致、首行magic、唯一非空command、唯一可解析UTC/offset started、唯一整数exit且为末非空行，逐项缺失/重复/mismatch/非法started/非终端exit负例均失败，至少一份严格信封exit0才通过；普通label只精确接受`t<N>-red|green`，`redo/red-extra/greenish/green-extra`及`t500-*`均不可冒充；仍要求done，缺日志/全非0/截图/其他task/suite-smoke-demo失败；文档明确strict envelope非密码学防伪、完整人工仿造仍属编辑evidence造假；同步.cursor/.opencode两套gate、单测、hf-workflow/hf-build文档且镜像一致，不新增第二个最终验证任务

任务覆盖索引：FR-1→T-2/3/17/20/27/28/31/40/48/50；FR-2→T-1/2/5/16/22–26/28/30–31/40/43–48/50；FR-3→T-8–10/12/14–19/28/31–32/37/41/44/48/50；FR-4→T-6–10/16/31/33–34/36–37/40/49–50；FR-5→T-6/7/10/16/31/34–37/40–41/49–50；FR-6→T-11–13/16/19/25/27/29/31/43–44/50；FR-7→T-19/20/26/29/31/37/41–42/45/50；FR-8→T-7/11/15/18–23/29/31/33–42/44–46/48–50；FR-9→T-19–23/25/29/30–31/33/35/38–39/45–48/50；FR-10→T-1/5/6/8–14/17/19/24/26/28/31–32/36/40–41/43/46–47/49–50；FR-11→T-1/2/4/5/7/8/10/13/14/16/18/21–26/31/36–50；FR-12→T-4/5/18/20/23–25/28/30–31/43/46/50；FR-13→T-1/4/5/9/11–15/18/22–26/29/31/43–47/50；FR-14→T-2/11/27–30/45–48/50。NFR-1→T-1–3/5/7/10/16–24/28/30–50；NFR-2→T-6–17/19/21/26/29/31/33–41/43/46–47/49–50；NFR-3→T-6/8–15/25/27/29/30–35/37/43–44/46–47/49–50；NFR-4→T-1/2/4/5/7–9/11–18/20–26/31/36–51；NFR-5→T-2/27–30/45–48/50。
