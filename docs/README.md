...
- [项目产品定义](../product/product.md)与[切片历史](../features/)：分别查看产品边界和已交付切片的规格、设计与验收记录。

## 文档事实边界

- 根目录 [README](../README.md) 是仓库入口与最短安装/运行说明，不承担完整产品手册。
- `docs/` 是当前用户与开发者文档，解释如何使用、配置、验证及理解已实现行为。
- [`product/`](../product/) 是产品层事实源：产品边界、已确认决策、假设台账和切片待办；用户文档不能反向替代这些记录。
...
[协作驾驶舱总览](../product/ui/cool-ai-design-md-case.html) · [设计规范](../product/ui/DESIGN.md)
